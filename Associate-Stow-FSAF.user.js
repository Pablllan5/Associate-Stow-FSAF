...
(function () {
    ...
    const STORE_KEY = 'pab_stow_audit_api_v430';
    const BTN_ID = 'pab-stow-fast-btn';
    const PANEL_ID = 'pab-stow-fast-panel';
    const BREAK_LIMIT_MS = 9 * 60 * 1000;
    const BREAK_VISIBLE_MAX_MS = 30 * 60 * 1000;
    const MAX_CONCURRENT = 5;

    const clean = v => String(v || '').replace(/\u00a0/g, ' ').trim();
    function getStore() { return GM_getValue(STORE_KEY, { running:false, selected:{}, associates:[], results:{}, sortBy:null, sortDir:'desc', startDate:null, endDate:null, workerComments:{}, sblIssues:'', showOnlyActive:false, webhookUrl:'', webhookMeetingUrl:'' }); }
    ...
    async function apiPost(body) {
        ...
        const res = await fetch(`${origin}/station/proxyapigateway/data`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/json;charset=UTF-8','X-API-Usage-Key':getApiKey(),'X-Requested-With':'XMLHttpRequest'}, body:JSON.stringify(body) });
    ...
    async function fetchAllPackages(login, startTime, endTime) {
        ...
        do { const d=await apiPost({resourcePath:"/os/getPackageHistoryForAssociate",httpMethod:"post",processName:"oculus",requestBody:{associateLoginId:login.includes('@')?login:`${login}@amazon.com`,pageSize:100,pageToken:pt,startTime,endTime}}); all=all.concat(d.packageRecordList||[]); pt=d.nextPageToken||null; if(++pg>80)break; } while(pt);
        return all;
    }
    ...
    function pairAisles(aisleMap){
        // Group by letter, then pair numbers 1/2, 3/4, 5/6... 98/99
        const byLetter={};
        ...
        Object.entries(byLetter).sort().forEach(([letter,nums])=>{
            ...
            const used=new Set();
            // Pair 1/2, 3/4, 5/6... 98/99
            for(let i=0;i<numbers.length;i++){
                const n=numbers[i];
                if(used.has(n))continue;
                const partner=n%2===1?n+1:n-1; // odd pairs with next even
                if(nums[partner]!==undefined&&!used.has(partner)){
                    ...
                    pairs.push({label:`${letter}${n}`,count:nums[n]||0});
                    used.add(n);
                }
            }
        });
    ...
    function calcTime(ts,cnt){
        ...
        if(s.length<2)return{pph:0,excludedBreakMinutes:0};
        let ms=0,brk=0;
        for(let i=1;i<s.length;i++){const d=s[i]-s[i-1];if(d>0&&d<=BREAK_LIMIT_MS)ms+=d;else if(d>BREAK_LIMIT_MS)brk+=d/60000;}
        const min=ms/60000;return{pph:min>0?cnt/(min/60):0,excludedBreakMinutes:brk};
    }
    ...
    async function fetchAAs(){const sc=getStation(),cy=getCycle();if(!sc)throw new Error('No station.');const d=await apiPost({resourcePath:"svs/associates/data",httpMethod:"post",processName:"stow",requestBody:{filters:{"NODE":[sc],"CYCLE":[cy]},fieldsRequired:["NAME","STATUS","PERFORMANCE","LOCATION"]}});if(!d?.associates)return[];return d.associates.map(a=>({associate:a.alias?a.alias.split('@')[0]:'',active:(a.status||'').toUpperCase()==='ACTIVE'})).filter(x=>x.associate&&/^[a-z][a-z0-9_.-]{2,}$/i.test(x.associate));}

    function parseSbl(v){return String(v||'').split(/[,;\n]+/).map(x=>clean(x).toUpperCase()).filter(Boolean);}
    function sblStats(rows,sbl){const list=parseSbl(sbl);if(!list.length)return{count:0,total:0,pct:0};const all=rows.flatMap(r=>(r.details||[]));const aff=all.filter(d=>d.type==='sourceZone'&&list.some(c=>(d.ref||'').toUpperCase().startsWith(c)));const tot=rows.reduce((a,r)=>a+(r.errors||0),0);return{count:aff.length,total:tot,pct:tot>0?(aff.length/tot)*100:0};}

    async function runQueue(q,sd,ed,cb){const res={};let done=0;for(let i=0;i<q.length;i+=MAX_CONCURRENT){await Promise.all(q.slice(i,i+MAX_CONCURRENT).map(async l=>{try{const p=await fetchAllPackages(l,sd,ed);res[l]=p.length?analyze(p):{stowed:0,errors:0,sourceZoneErrors:0,boxErrors:0,trackingErrors:0,duplicateStowed:0,duplicateDetails:[],details:[],aisleBreakdown:{},pph:0,excludedBreakMinutes:0,error:'REVISAR'};}catch(e){
            // Auto-retry once
            try{await new Promise(r=>setTimeout(r,1500));const p2=await fetchAllPackages(l,sd,ed);res[l]=p2.length?analyze(p2):{stowed:0,errors:0,sourceZoneErrors:0,boxErrors:0,trackingErrors:0,duplicateStowed:0,duplicateDetails:[],details:[],aisleBreakdown:{},pph:0,excludedBreakMinutes:0,error:'REVISAR'};}catch(e2){res[l]={stowed:0,errors:0,sourceZoneErrors:0,boxErrors:0,trackingErrors:0,duplicateStowed:0,duplicateDetails:[],details:[],aisleBreakdown:{},pph:0,excludedBreakMinutes:0,error:'REVISAR',reason:e2.message};}
        }done++;if(cb)cb(l,done,q.length);}));if(!getStore().running)break;}return res;}
    ...
    function buildReport(){
        ...
        const hdrs=['ASSOCIATE','ACTIVO','STOWED','FSAF','DPMO','PPH','SZ','CAJA','TRACK','X2','STATUS','FEEDBACK'].join('\t');
        const xl=sorted.map(r=>[r.login,r.active?'Y':'N',r.stowed||0,r.errors||0,Math.round(calcDpmo(r.errors,r.stowed)),r.pph?Math.round(r.pph):'',r.sourceZoneErrors||0,r.boxErrors||0,r.trackingErrors||0,r.duplicateStowed||0,r.error||'OK',r.fb].join('\t')).join('\n');
        return`📦 STOW AUDIT\n${fmtNow()}\n\nStowed: ${t.s} | FSAF: ${t.e} | DPMO: ${Math.round(calcDpmo(t.e,t.s))} | X2: ${t.x2}\nSZ: ${t.sz} | Caja: ${t.bx} | Track: ${t.trk}\n\nEXCEL:\n${hdrs}\n${xl}`;
    }
    ...
    function buildMeeting(){
        ...
        const sbl=sblStats(rows,s.sblIssues||'');
        ...
        m+=`\n🔥 Top 3 — Más FSAF\n`;
        top3.forEach((r,i)=>{const dp=calcDpmo(r.errors,r.stowed);m+=`\n${i+1}. ${r.login}\n⚡ PPH: ${Math.round(r.pph||0)}\n📊 DPMO: ${Math.round(dp)}\n❌ FSAF: ${r.errors}\nSort Zone: ${r.sourceZoneErrors}\nCaja: ${r.boxErrors}\nTracking: ${r.trackingErrors}\nStowed X2: ${r.duplicateStowed}\n`;if(r.fb)m+=`💬 ${r.fb}\n`;});
        if(s.sblIssues&&sbl.count>0)m+=`\n🚧 SBL: ${s.sblIssues}\n⚠️ Afectados: ${sbl.count}/${sbl.total} (${sbl.pct.toFixed(0)}%)\n`;
        return m;
    }

    function copyR(){try{GM_setClipboard(buildReport());alert('Copiado.');}catch{prompt('Copia:',buildReport());}}
...
<label style="font-size:11px;color:#94a3b8;font-weight:800">SBL Aisles (ej: A17, C33)</label>
<input id="cfg-sbl" value="${s.sblIssues||''}" style="width:100%;background:#1e293b;border:1px solid #334155;color:white;border-radius:8px;padding:8px;margin:4px 0 16px;font-size:12px">
...
<div style="background:#1e293b;border-radius:10px;padding:10px;text-align:center"><div style="font-size:9px;color:#64748b;font-weight:800">PPH</div><div style="font-size:20px;font-weight:900;margin-top:4px;color:#22c55e">${r?.pph?r.pph.toFixed(0):'-'}</div></div>
...
#${PANEL_ID} .bo{background:#ff9900;color:#111;border-color:#ff9900} #${PANEL_ID} .bg{background:#14532d;border-color:#22c55e;color:#86efac} #${PANEL_ID} .br{background:#7f1d1d;border-color:#ef4444;color:#fca5a5} #${PANEL_ID} .bgy{background:#334155;color:#cbd5e1;border-color:#475569} #${PANEL_ID} .bp{background:#831843;border-color:#db2777;color:#f9a8d4} #${PANEL_ID} .bcfg{background:#334155;color:#94a3b8;border-color:#475569} #${PANEL_ID} .bx{background:linear-gradient(135deg,#1e40af,#3b82f6);border:0;color:white;padding:8px 14px}
#${PANEL_ID} table{width:100%;border-collapse:collapse;font-size:12px} #${PANEL_ID} th{text-align:left;color:#64748b;font-size:9px;text-transform:uppercase;border-bottom:1px solid rgba(255,255,255,.1);padding:7px 5px;white-space:nowrap} #${PANEL_ID} th.st{cursor:pointer;color:#fbbf24} #${PANEL_ID} td{border-bottom:1px solid rgba(255,255,255,.05);padding:7px 5px;color:#e2e8f0} #${PANEL_ID} tbody tr:hover{background:rgba(255,255,255,.03)}
#${PANEL_ID} .sm{font-size:13px;font-weight:800;margin-bottom:8px;color:#ff9900} .bad{color:#ef4444;font-weight:900}.ok{color:#22c55e;font-weight:900}.warn{color:#facc15;font-weight:900}
#${PANEL_ID} td.an{color:#60a5fa;font-weight:900;font-size:13px} #${PANEL_ID} .pb{width:100%;height:8px;background:#1e293b;border-radius:4px;margin-top:8px;overflow:hidden} #${PANEL_ID} .pf{height:100%;background:linear-gradient(90deg,#ff9900,#f59e0b);border-radius:4px;transition:width .4s} #${PANEL_ID} input[type="checkbox"]{transform:scale(1.2);accent-color:#ff9900}`;
    ...
    function showPanel(){
        ...
        const s=getStore(),base=s.showOnlyActive?(s.associates||[]).filter(a=>a.active):(s.associates||[]);
        const shown=sortAssociates(base,s);
        const tE=Object.values(s.results||{}).reduce((a,r)=>a+(r.errors||0),0),tS=Object.values(s.results||{}).reduce((a,r)=>a+(r.stowed||0),0),tX=Object.values(s.results||{}).reduce((a,r)=>a+(r.duplicateStowed||0),0);
...
<button id="z-sa" class="bgy">Sel. todos</button><button id="z-sn" class="bgy">Limpiar sel.</button>
<button id="z-del" class="br">Borrar</button>
<button id="z-cfg" class="bcfg">⚙️ Config</button>
<button id="z-ra" class="bo">Analizar activos</button><button id="z-rs" class="bo">Analizar sel.</button><button id="z-rt" class="bo">Analizar todos</button><button id="z-stop" class="br">Stop</button>
<button id="z-cp" class="bgy">Copiar</button><button id="z-sr" class="bp">Slack Report</button><button id="z-sm" class="bp">Slack Meeting</button>
<button id="z-exp" class="bx">⛶</button><button id="z-cls" class="bgy">✕</button>
</div></div>
<div class="sm">Estado: ${s.running?'Analizando...':'Parado'} · AAs: ${(s.associates||[]).length} · Stowed: ${tS} · FSAF: ${tE} · DPMO: ${Math.round(calcDpmo(tE,tS))} · X2: ${tX}${s.sblIssues?' · SBL: '+s.sblIssues:''}</div>
...
<tbody>${shown.length?shown.map(a=>{const r=s.results[a.associate],ck=s.selected?.[a.associate]?'checked':'',fb=s.workerComments?.[a.associate];return`<tr><td><input type="checkbox" class="pab-ck" data-l="${a.associate}" ${ck}></td><td class="${a.active?'ok':'bad'}">${a.active?'✅':'❌'}</td><td class="an"><img src="https://internal-cdn.amazon.com/badgephotos.amazon.com/?login=${a.associate}" style="width:26px;height:26px;border-radius:6px;vertical-align:middle;margin-right:5px;border:1px solid #334155" onerror="this.style.display='none'">${a.associate}${fb?' 📝':''}</td><td>${r?r.stowed:'-'}</td><td>${r?.pph?r.pph.toFixed(0):'-'}</td><td>${r?Math.round(calcDpmo(r.errors,r.stowed)):'-'}</td><td class="${r?.errors>0?'bad':'ok'}">${r?r.errors:'-'}</td><td>${r?r.sourceZoneErrors:'-'}</td><td>${r?r.boxErrors:'-'}</td><td>${r?r.trackingErrors:'-'}</td><td class="${r?.duplicateStowed>0?'warn':'ok'}">${r?r.duplicateStowed:'-'}</td><td class="${r?.error?'warn':'ok'}">${r?(r.error||'OK'):'-'}</td><td><button class="pab-1" data-l="${a.associate}">▶</button><button class="pab-i" data-l="${a.associate}">Info</button><button class="pab-p" data-l="${a.associate}">⏸</button><button class="pab-fb" data-l="${a.associate}">💬</button></td></tr>`;}).join(''):'<tr><td colspan="13">Pulsa Actualizar.</td></tr>'}</tbody></table>`;
        document.body.appendChild(p);
        if(_isFullscreen) p.classList.add('fs');
        const $=id=>document.getElementById(id);
        $('z-cls').onclick=()=>{_isFullscreen=false;p.remove();};$('z-exp').onclick=()=>{_isFullscreen=!_isFullscreen;p.classList.toggle('fs');$('z-exp').textContent=_isFullscreen?'⊡':'⛶';};
        $('z-cp').onclick=copyR;$('z-sr').onclick=sendR;$('z-sm').onclick=sendM;$('z-cfg').onclick=showConfig;
        ...
        $('z-sa').onclick=()=>{const st=getStore();st.selected={};st.associates.forEach(a=>st.selected[a.associate]=true);setStore(st);showPanel();};
        $('z-sn').onclick=()=>{const st=getStore();st.selected={};setStore(st);showPanel();};
        ...
        $('z-rt').onclick=()=>startRun(getStore().associates.map(a=>a.associate));
        $('z-ra').onclick=()=>{const a=getStore().associates.filter(x=>x.active).map(x=>x.associate);if(!a.length)return alert('No activos.');startRun(a);};
        $('z-rs').onclick=()=>{const st=getStore(),a=st.associates.filter(x=>st.selected?.[x.associate]).map(x=>x.associate);if(!a.length)return alert('Sel. vacía.');startRun(a);};
        ...
        p.querySelectorAll('.pab-p').forEach(b=>{b.onclick=()=>{const r=getStore().results?.[b.dataset.l];const brks=r?.excludedBreaks||r?.excludedBreakMinutes;if(!brks&&!(r?.excludedBreakMinutes>0)){alert(`${b.dataset.l}: Sin pausas 9-30 min detectadas.`);return;}alert(`Pausas 9-30 min — ${b.dataset.l}\n\nTiempo total excluido: ${Math.round(r.excludedBreakMinutes||0)} min`);};});
